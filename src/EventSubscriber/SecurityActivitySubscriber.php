...
final readonly class SecurityActivitySubscriber implements EventSubscriberInterface
{
    ...
    public static function getSubscribedEvents(): array
    {
        return [
            ...
            // Before Security's own Firewall\ExceptionListener (priority 1), which replaces the exception.
    ...
    {
        try {
            // getToken() is the new token when entering impersonation, but the restored one when exiting it.
    ...
    /**
     * Fired by Security's ContextListener when the user reloaded from the session no longer matches the token.
     */
    ...
    /**
     * A 403 for an anonymous visitor is the firewall sending them to the login page, not a signal.
     */
