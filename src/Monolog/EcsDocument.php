...
/**
 * The pure Activity-to-ECS-document mapping, reused by EcsFormatter and by EcsSchemaTest.
 *
 * @internal
 *
 ...
 * @phpstan-type EcsHost array{hostname: string}
 ...
 * @phpstan-type EcsSubject array{type: string, id: string, owner?: string}
 ...
 * @phpstan-type EcsDocumentShape array{'@timestamp': string, ecs: array{version: string}, event: EcsEvent, service?: array{name?: string, environment?: string}, host?: EcsHost, user?: EcsUser, user_agent?: array{original: string}, source?: EcsSource, http?: EcsHttp, url?: EcsUrl, related?: EcsRelated, vigie: EcsVigie}
 */
final class EcsDocument
{
    public const ECS_VERSION = '8.11.0';

    // DateTimeInterface::ATOM would drop the microseconds.
    private const DATE_FORMAT = 'Y-m-d\TH:i:s.uP';

    // Matches a hash_hmac('sha256', ...) output, to tell a pseudonymized value from one kept in the clear.
     ...
     */
    public static function fromActivity(Activity $activity, \DateTimeImmutable $recordedAt, ?string $app = null, ?string $env = null, ?string $hostname = null): array
    {
        ...
        if (null !== $hostname) {
            $document['host'] = ['hostname' => $hostname];
        }
    ...
    private static function vigie(Activity $activity): array
    {
        ...
        if (null !== $activity->subject) {
            $subject = ['type' => $activity->subject->type, 'id' => $activity->subject->id];

            if (null !== $activity->subject->owner) {
                $subject['owner'] = $activity->subject->owner;
            }

            $vigie['subject'] = $subject;
        }

        if ([] !== $activity->context) {
            // Cast to object, so a context with numeric-string keys ("404") doesn't encode as a JSON array.
    ...
    private static function user(Activity $activity): array
    {
        ...
        // context.impersonator (TokenProcessor) or context.original_user (switch_user): the real actor behind user.name.
    ...
    private static function related(Activity $activity): array
    {
        ...
        $users = [];
        foreach ([$activity->userIdentifier, self::contextString($activity, 'impersonator'), self::contextString($activity, 'original_user'), $activity->subject?->owner] as $candidate) {
            if (null !== $candidate) {
                $users[] = $candidate;
            }
        }
