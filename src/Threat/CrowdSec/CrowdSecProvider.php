 ...
 * authenticated with a bouncer API key (`cscli bouncers add`). Read-only:
 * it pulls decisions, it never pushes one back.
 *
 * @internal
 */
final readonly class CrowdSecProvider implements ThreatProviderInterface
{
    ...
    // The LAPI derives per-bouncer metrics from this header's shape.
    private const USER_AGENT = 'crowdsec-symfony-vigie-bouncer/v1';

    /**
     * @param list<string> $scopes              sent explicitly, so a scope configured here isn't silently dropped by the LAPI's own default
    ...
    private function toBatch(string $content): ThreatSyncBatch
    {
        ...
        // A stream with nothing to report answers a literal JSON "null" body, not "[]" or "{}".
    ...
    private static function decisionFromArray(array $raw, \DateTimeImmutable $now): ?ThreatDecision
    {
        ...
        // Treated as already expired, never clamped to a minimum, which would produce a ban nothing purges.
        ...
        // DateInterval has no fractional-second component, so a sub-second duration (e.g. "300ms") is rounded up.
