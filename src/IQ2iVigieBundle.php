...
final class IQ2iVigieBundle extends AbstractBundle
{
    ...
    public function configure(DefinitionConfigurator $definition): void
    {
        ...
        $rootNode
            ->children()
                ...
                ->scalarNode('app')
                    ->defaultNull()
                    ->info('Stamped as "app" on every recorded line. Null omits it.')
                ->end()
                ->scalarNode('env')
                    ->defaultValue('%kernel.environment%')
                    ->info('Stamped as "env" on every recorded line, the same way "app" is. Defaults to the current kernel environment.')
                ->end()
                ->variableNode('hostname')->defaultTrue()
                    ->info('Stamped as "host.hostname" on every recorded line. true resolves gethostname() at runtime, false omits the field, a string sets it explicitly (e.g. "%env(HOSTNAME)%"). See doc/multi-server.md.')
                    ->validate()
                        ->ifTrue(static fn (mixed $v): bool => !\is_bool($v) && (!\is_string($v) || '' === trim($v)))
                        ->thenInvalid('iq2i_vigie.hostname must be true, false, or a non-empty string, got %s.')
                    ->end()
                ->end()
                ->scalarNode('storage')
                    ->defaultNull()
                    ->info('Service id implementing ActivityStorageInterface, or "in_memory" for the bundle\'s in-memory test double. Defaults to "monolog": activities are logged as ECS documents through a dedicated Monolog channel. See doc/storage.md.')
                ...
                ->arrayNode('output')
                    ...
                    ->children()
                        ->scalarNode('path')
                            ...
                            ->cannotBeEmpty()
                            ->info('Stream URL the default handler writes NDJSON to when "handlers" is empty (an absolute path, or e.g. "php://stdout"). Ignored once "handlers" is non-empty.')
                        ->end()
                        ->arrayNode('handlers')
                            ->info('Service ids implementing Monolog\Handler\HandlerInterface. Empty (the default) makes Vigie create a single StreamHandler on "path". Once set, configure each handler\'s own formatter (e.g. \'iq2i_vigie.formatter.ecs\'). See doc/storage.md.')
                            ->scalarPrototype()->end()
                            ->defaultValue([])
                        ->end()
                    ->end()
                ...
                ->arrayNode('record')
                    ->addDefaultsIfNotSet()
                    ->info('Individually toggle which fields get recorded. A disabled field is nulled out before the activity ever reaches a storage or an ActivityRecording listener.')
                    ->children()
                        ->variableNode('ip_address')->defaultValue('anonymize')
                            ->info('true, false, or "anonymize" (the default) to mask the host part (/24 for IPv4, /64 for IPv6) instead of dropping it.')
                            ->validate()
                        ...
                        ->booleanNode('query_string')->defaultFalse()
                            ->info('Whether the query string is kept as part of "uri". It routinely carries tokens (password resets, OAuth codes) that have no business in a long-lived record.')
                        ->end()
                ...
                ->arrayNode('http')
                    ...
                    ->children()
                        ...
                        ->arrayNode('ignored_paths')
                            ->info('Regular expressions (without delimiters) matched against the request path; a match is never recorded, an exception carved out of recorded_paths. A #[Track]/#[Untrack] attribute or a tagged ActivityDeciderInterface takes precedence over it.')
                            ->scalarPrototype()->end()
                            ->defaultValue([])
                            ->validate()->always(self::validatePatterns('http.ignored_paths'))->end()
                        ->end()
                        ->arrayNode('recorded_paths')
                            ->info('Regular expressions (without delimiters) matched against the request path. Nothing is recorded by default; a path is recorded only once it matches one of these patterns. Use #[Track] to opt in a controller instead.')
                            ->scalarPrototype()->end()
                            ->defaultValue([])
                ...
                ->arrayNode('threat')
                    ->addDefaultsIfNotSet()
                    ->info('Ingests the decisions a SIEM (CrowdSec today) hands back about suspicious IPs, ranges, sessions, users, countries and AS numbers, and exposes them through ThreatCheckerInterface. See doc/threat.md.')
                    ->children()
                        ...
                        ->arrayNode('cache')
                            ...
                            ->children()
                                ->scalarNode('pool')
                                    ...
                                    ->cannotBeEmpty()
                                    ->info('Service id of a PSR-6 CacheItemPoolInterface, holding the decisions when threat.storage is "cache" and the accepted ingest signatures when threat.ingest is on.')
                                ->end()
                            ->end()
                        ->end()
                        ->arrayNode('match')
                            ...
                            ->children()
                                ->booleanNode('normalize_subject')->defaultTrue()
                                    ->info('Whether a session id and a user identifier are pushed through QueryNormalizer before being looked up. Never applies to the IP, which is always matched raw.')
                                ->end()
                        ...
                        ->arrayNode('enforce')
                            ->addDefaultsIfNotSet()
                            ->info('Opt-in enforcement: turns the highest decision matching a request into a response, on kernel.request just under the firewall. Off by default. See doc/threat.md.')
                            ->children()
                                ->booleanNode('enabled')->defaultFalse()->end()
                                ->arrayNode('remediations')
                                    ->info('Remediation type ("ban", "captcha", any type a scenario invents) mapped to either an HTTP status code (400-599) or the name of a route to redirect to. A type absent from this map is never acted on, but ThreatDecisionMatched is still dispatched.')
                                    ->variablePrototype()->end()
                                    ->defaultValue([])
                                    ->validate()->always(self::validateRemediations())->end()
                                ->end()
                                ->arrayNode('exclude_paths')
                                    ->info('Regular expressions (without delimiters) matched against the request path; a match is never enforced (health checks, payment webhooks).')
                                    ->scalarPrototype()->end()
                                    ->defaultValue([])
                        ...
                        ->arrayNode('ingest')
                            ->addDefaultsIfNotSet()
                            ->info('HTTP endpoint a SIEM pushes decisions to, the reverse of polling one with vigie:threat:sync. Nothing is reachable until the application imports "@IQ2iVigieBundle/config/routes.php".')
                            ->children()
                                ->booleanNode('enabled')->defaultFalse()->end()
                                ->arrayNode('providers')
                                    ->info('One shared secret per emitting SIEM, keyed by the provider name that appears in the URL. Use an env placeholder: "%env(VIGIE_INGEST_WAZUH_SECRET)%".')
                                    ->normalizeKeys(false)
                                    ->scalarPrototype()->end()
                                    ->defaultValue([])
                                ...
                                ->integerNode('max_body_size')->defaultValue(1048576)->min(1)
                                    ->info('Largest accepted request body, in bytes. Anything above is refused with a 413 before the payload is parsed.')
                                ->end()
                        ...
                        ->arrayNode('crowdsec')
                            ...
                            ->children()
                                ...
                                ->arrayNode('scopes')
                                    ...
                                    ->info('Scopes requested from the LAPI stream, sent verbatim.')
                                ->end()
                                ->arrayNode('origins')
                                    ->scalarPrototype()->end()
                                    ->defaultValue([])
    ...
    public function loadExtension(array $config, ContainerConfigurator $container, ContainerBuilder $builder): void
    {
        ...
        if ('monolog' === $storage) {
            ...
            $container->services()
                // Its own channel, not declared to monolog-bundle, or ECS lines would end up in prod.log too.
        ...
        if (!$threatConfig['enabled'] && $threatConfig['enforce']['enabled']) {
            throw new \LogicException('iq2i_vigie.threat.enforce.enabled is true but iq2i_vigie.threat.enabled is false. Set "iq2i_vigie.threat.enabled: true" as well.');
        }

        if (!$threatConfig['enabled'] && $threatConfig['ingest']['enabled']) {
            throw new \LogicException('iq2i_vigie.threat.ingest.enabled is true but iq2i_vigie.threat.enabled is false. The ingest endpoint writes into the threat store, which only exists once the subsystem is on.');
        }
    ...
    private function loadThreatExtension(array $threatConfig, ContainerConfigurator $container, ContainerBuilder $builder): void
    {
        ...
        if ($ingestConfig['enabled']) {
            if ([] === $ingestConfig['providers']) {
                throw new \LogicException('iq2i_vigie.threat.ingest.enabled is true but "iq2i_vigie.threat.ingest.providers" is empty. Declare one shared secret per emitting SIEM.');
            }
        ...
        if ('crowdsec' === $threatConfig['provider']) {
            ...
            if (null === $crowdsecConfig['api_key']) {
                throw new \LogicException('iq2i_vigie.threat.provider is set to "crowdsec" but "iq2i_vigie.threat.crowdsec.api_key" is not set. Create one with "cscli bouncers add <name>".');
            }
