...
final class EcsFormatterTest extends TestCase
{
    ...
    {
        // "@timestamp" is set to occurredAt; the formatter must not overwrite it with $record->datetime.
    ...
    public function testAppendNewlineFalseProducesNoTrailingNewline(): void
    {
        $record = $this->record(context: ['vigie' => ['type' => 'custom']]);

        $line = (new EcsFormatter(appendNewline: false))->format($record);
