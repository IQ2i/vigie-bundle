...
final class EcsDocumentTest extends TestCase
{
    ...
    public function testSubjectOwnerMapsToVigieSubjectOwner(): void
    {
        $document = EcsDocument::fromActivity(
            Activity::httpRequest(new \DateTimeImmutable(), 'GET', '/orders/42', 200, subject: new Subject('order', 42, owner: 'jane.doe')),
            new \DateTimeImmutable(self::RECORDED_AT),
        );

        self::assertSame(['type' => 'order', 'id' => '42', 'owner' => 'jane.doe'], $document['vigie']['subject'] ?? null);
    }

    public function testSubjectOwnerIsAddedToRelatedUser(): void
    {
        $document = EcsDocument::fromActivity(
            Activity::httpRequest(new \DateTimeImmutable(), 'GET', '/orders/42', 200, userIdentifier: 'attacker', subject: new Subject('order', 42, owner: 'jane.doe')),
            new \DateTimeImmutable(self::RECORDED_AT),
        );

        self::assertContains('attacker', $document['related']['user'] ?? []);
        self::assertContains('jane.doe', $document['related']['user'] ?? []);
    }
    ...
    public function testHostnameMapsToHostHostname(): void
    {
        $document = EcsDocument::fromActivity(
            Activity::custom('export.completed', new \DateTimeImmutable()),
            new \DateTimeImmutable(self::RECORDED_AT),
        ...
        self::assertSame(['hostname' => 'web-1'], $document['host'] ?? null);
    }

    public function testNoHostnameOmitsHost(): void
    {
        $document = EcsDocument::fromActivity(
            Activity::custom('export.completed', new \DateTimeImmutable()),
            new \DateTimeImmutable(self::RECORDED_AT),
        ...
        self::assertArrayNotHasKey('host', $document);
    }
