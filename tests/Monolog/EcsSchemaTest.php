...
final class EcsSchemaTest extends TestCase
{
    ...
    public static function activityProvider(): iterable
    {
        ...
        yield 'http_request' => [Activity::httpRequest(
            ...
            requestId: 'req-1',
            subject: new Subject('order', 42, owner: 'john.doe'),
        )];
    ...
    {
        // "message" isn't part of EcsDocument::fromActivity()'s own output, but it's still part of this contract.
        $document = EcsDocument::fromActivity($activity, new \DateTimeImmutable('2026-08-21 10:00:01'), 'shop', 'prod', 'web-1')
    ...
    public function testEveryActivityTypeIsCoveredByTheVigieTypeEnum(): void
    {
        ...
        foreach (ActivityType::cases() as $case) {
            self::assertContains($case->value, $enum, \sprintf('ActivityType::%s has no counterpart in the "vigie.type" enum. Update doc/schema/activity.json.', $case->name));
        }
    }
