...
final class SecurityFlowTest extends FunctionalTestCase
{
    ...
    public function testASwitchUserIsRecordedWithTheOriginalUser(): void
    {
        ...
        // A failed impersonation responds 403 instead, so the redirect itself is the success signal.
    ...
    public function testALogoutIsRecordedWithTheFirewallAndUser(): void
    {
        ...
        // http_basic re-authenticates from the header on every request, so it must still be present here.
        $client->request('GET', '/logout', server: ['PHP_AUTH_USER' => 'jane.doe', 'PHP_AUTH_PW' => 'password']);

        // Symfony's logout listener redirects to the configured target ("/" by default).
        self::assertSame(302, $client->getResponse()->getStatusCode());
    ...
    // The "exit" direction is covered at the unit level: this micro test kernel doesn't persist sessions across requests.
