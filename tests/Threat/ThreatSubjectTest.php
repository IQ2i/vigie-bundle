...
final class ThreatSubjectTest extends TestCase
{
    ...
    public function testNetworkReadsIpCountryAndAsnButNotSessionOrUserIdentifier(): void
    {
        $request = Request::create('/');
        $request->server->set('REMOTE_ADDR', '203.0.113.42');
        $request->headers->set('Cf-IPCountry', 'FR');
        ...
        $subject = ThreatSubject::network($request, countryHeader: 'Cf-IPCountry', asnHeader: 'X-Asn');

        self::assertSame('203.0.113.42', $subject->ip);
        self::assertSame('FR', $subject->country);
        self::assertSame('1234', $subject->asn);
        self::assertNull($subject->sessionId);
        self::assertNull($subject->userIdentifier);
    }

    public function testIdentityReadsSessionAndUserIdentifierButNotIpCountryOrAsn(): void
    {
        $request = Request::create('/');
        $request->server->set('REMOTE_ADDR', '203.0.113.42');

        $subject = ThreatSubject::identity($request, userIdentifier: 'jane.doe');

        self::assertSame('jane.doe', $subject->userIdentifier);
        self::assertNull($subject->ip);
        self::assertNull($subject->country);
        self::assertNull($subject->asn);
    }
}
