...
final class HttpActivitySubscriberTest extends TestCase
{
    public function testItRecordsTheHttpRequestOnTerminate(): void
    {
        ...
        // Nothing is recorded on kernel.response, only captured; "expects(once)" is satisfied by the terminate call.
    ...
    {
        // In a worker runtime, a request that returns early from onKernelResponse() must not leave a stale $pending.
        $recorder = $this->createMock(ActivityRecorderInterface::class);
        $recorder->expects(self::never())->method('record');
    ...
    public function testResetClearsThePendingActivity(): void
    {
        ...
        // Simulates a worker runtime resetting every ResetInterface service between requests.
        $subscriber->reset();
