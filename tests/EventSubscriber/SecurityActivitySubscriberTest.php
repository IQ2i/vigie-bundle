...
final class SecurityActivitySubscriberTest extends TestCase
{
    ...
    public function testANonInteractiveAuthenticatorIsRecordedAsSuch(): void
    {
        ...
        // A mocked AuthenticatorInterface doesn't implement InteractiveAuthenticatorInterface, like a stateless API token authenticator.
        $authenticator = $this->createMock(AuthenticatorInterface::class);
        $token = new UsernamePasswordToken(new InMemoryUser('jane.doe', null), 'main');
    ...
    public function testSwitchUser(): void
    {
        ...
        // SwitchUserListener wraps the impersonator's own token; a bare token here would mean "exiting" instead.
    ...
    public function testSwitchUserExitDoesNotReportAnOriginalUser(): void
    {
        ...
        // Exiting impersonation: the event carries the restored original token directly, not a SwitchUserToken.
...
/**
 * Stands in for the entity a voter denied access to; only its class name ends up in context.subject_type.
 */
