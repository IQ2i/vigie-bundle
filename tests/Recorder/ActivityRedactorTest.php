...
final class ActivityRedactorTest extends TestCase
{
    ...
    public function testIpAddressAnonymizeKeepsTheNetworkPrefix(): void
    {
        ...
        // The last 8 bytes (a /64) are zeroed; the network prefix survives.
    ...
    public function testSubjectOwnerGetsTheSameModeAndSecretAsUserIdentifier(): void
    {
        $activity = $this->activity()->withSubject(new Subject('order', 42, owner: 'jane.doe'));

        $result = $this->redactor(new RecordingOptions(userIdentifier: 'hash'), 'secret-a')->redact($activity);

        self::assertNotNull($result->subject);
        self::assertSame('order', $result->subject->type);
        self::assertSame('42', $result->subject->id);
        self::assertSame($result->userIdentifier, $result->subject->owner);
    }

    public function testSubjectOwnerIsDroppedWhenUserIdentifierIsDisabled(): void
    {
        $activity = $this->activity()->withSubject(new Subject('order', 42, owner: 'jane.doe'));

        $result = $this->redactor(new RecordingOptions(userIdentifier: false))->redact($activity);

        self::assertNotNull($result->subject);
        self::assertNull($result->subject->owner);
    }

    public function testSubjectWithoutOwnerIsUnaffected(): void
    {
        $activity = $this->activity()->withSubject(new Subject('order', 42));

        $result = $this->redactor(new RecordingOptions())->redact($activity);

        self::assertNotNull($result->subject);
        self::assertNull($result->subject->owner);
    }

    public function testNullSubjectStaysNull(): void
    {
        $result = $this->redactor(new RecordingOptions())->redact($this->activity());
