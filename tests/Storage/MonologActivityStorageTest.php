...
final class MonologActivityStorageTest extends TestCase
{
    ...
    public function testHostnameTrueResolvesGethostname(): void
    {
        ...
        $logger->expects(self::once())
            ->method('info')
            ->with(self::anything(), self::callback(static function (array $document): bool {
                /** @var EcsDocumentShape $typed */
                $typed = $document;
                self::assertSame(gethostname(), $typed['host']['hostname'] ?? null);

                return true;
            }));

        $storage = new MonologActivityStorage($logger, hostname: true);
        $storage->store($activity);
    }

    public function testHostnameFalseOmitsHost(): void
    {
        $activity = Activity::custom('export.completed', new \DateTimeImmutable());
        ...
        $storage = new MonologActivityStorage($logger, hostname: false);
        $storage->store($activity);
    }

    public function testHostnameAsAStringIsUsedVerbatim(): void
    {
        $activity = Activity::custom('export.completed', new \DateTimeImmutable());
        ...
        $logger->expects(self::once())
            ...
            ->with(self::anything(), self::callback(static function (array $document): bool {
                /** @var EcsDocumentShape $typed */
                $typed = $document;
                self::assertSame('web-1', $typed['host']['hostname'] ?? null);

                return true;
            }));

        $storage = new MonologActivityStorage($logger, hostname: 'web-1');
        $storage->store($activity);
    }
