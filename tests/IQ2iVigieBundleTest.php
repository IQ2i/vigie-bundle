...
final class IQ2iVigieBundleTest extends TestCase
{
    ...
    public function testHostnameDefaultsToTrueAndIsPassedToTheMonologStorage(): void
    {
        $builder = $this->compiledContainer([]);

        self::assertTrue($builder->getDefinition('iq2i_vigie.monolog_storage')->getArgument('$hostname'));
    }

    public function testANonBooleanNonStringHostnameIsRejectedAsAConfigError(): void
    {
        $this->expectException(\Symfony\Component\Config\Definition\Exception\InvalidConfigurationException::class);
        $this->expectExceptionMessageMatches('/iq2i_vigie\.hostname must be true, false, or a non-empty string/');

        $this->compiledContainer(['hostname' => 42]);
    }
