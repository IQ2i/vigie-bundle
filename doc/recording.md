...
5. The default: nothing is recorded.
...
`IQ2i\VigieBundle\Model\Subject` is `type`/`id`/`owner`: `type` a free-form string you choose
(`"user"`, `"order"`), `id` a `string|int` always stored as a string, `owner` the user identifier of
whoever owns the resource, when your domain has that notion and you know it:
...
$this->recorder->custom('order.viewed', subject: new Subject('order', $order->getId(), owner: $order->getOwner()->getUserIdentifier()));
```

Emitted as `vigie.subject.type`/`vigie.subject.id`/`vigie.subject.owner` in the ECS document, see
[doc/siem.md](siem.md). `owner` exists to make an IDOR detectable: a plain `http_request` 200 on
`/orders/{id}` looks identical whether the caller owns that order or not. Pairing every access with
its `subject.owner` lets a scenario flag `subject.owner != user.name` (or `user.hash`) instead of
requiring a dedicated `access_denied` your application never raises, because the request *does*
succeed.

`owner` is a user identifier, so it goes through the exact same redaction as `userIdentifier`
(`record.user_identifier`), with the same secret: `user.hash` and `vigie.subject.owner` are HMACed
identically, so a scenario can compare them for equality whether or not identifiers are recorded in
the clear. There is no separate `record.*` toggle for it.
...
Suggested `context` keys: `roles_changed`, `added` and `removed` (each joined by `,` for more than one
role), `by` (the admin who made the change, when different from the user); `password_changed`, `reason`
...
The bundle registers two, both filling a field or context key only when the activity doesn't already carry
one:
