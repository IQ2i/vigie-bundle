...
A ban on `Ip`/`Range` has no recipe here: enforcement (`threat.enforce.remediations`) is the better fit,
...
`ThreatDecisionMatched` only ever carries a `session`-scoped decision from the **identity** stage of
`ThreatEnforcementSubscriber` (under the firewall, once the session exists): the network stage, above the
firewall, never looks up a session. This listener doesn't need to tell the two apart itself, it just never
fires for anything else, see [doc/threat.md](threat.md#enforcing-a-decision).
...
revoke directly. A version counter on the user, bumped once per ban and compared on the next
request, either `EquatableInterface::isEqualTo()` or `UserCheckerInterface::checkPostAuth()` throwing when
the stored version is behind, handles it instead. `ContextListener` then deauthenticates the token on its own, producing a
...
When the SIEM authored its decision against Vigie's own ECS output with `record.user_identifier: hash`,
`$decision->value` is the HMAC `user.hash` carried in that output, not the identifier itself. Keep a column
on the user, indexed, holding that same hash, looked up directly rather than iterating every user to compare
hashes. Compute it with `QueryNormalizer::userIdentifier()` (autowireable, applies the same `record.*`
transformation `ActivityRedactor` used when Vigie itself last wrote the value) whenever the identifier is
set, not `Pseudonymizer`, which is `@internal` and not meant to be reached directly:

```php
use IQ2i\VigieBundle\Recorder\QueryNormalizer;

final readonly class UpdateUserLookupHash
{
    public function __construct(
    ...
    {
        return $this->normalizer->userIdentifier($userIdentifier); // store this in the indexed column
    }
}
```
