...
iq2i_vigie:
    ...
    # document, so an external consumer reading several applications' or
    # environments' streams can tell them apart. "app" defaults to null
    # (omitted); "env" defaults to the current kernel environment.
    app: null
    env: '%kernel.environment%'

    # Stamped as "host.hostname" on every recorded line, so the servers of a
    # load-balanced deployment can be told apart in a shared SIEM. See
    # doc/multi-server.md. true (the default) resolves gethostname() at
    # runtime, never at container compile time; a string sets it explicitly
    # (e.g. "%env(HOSTNAME)%"); false omits the field.
    hostname: true

    # Service id implementing ActivityStorageInterface, or "in_memory" for
    ...
    output:
        ...
        # with its own ECS formatter. See doc/storage.md for wiring a
        # syslog handler this way.
        handlers: []
    ...
    threat:
        ...
        enforce:
            enabled: false
            # ThreatChecker::highestFor() maps to an action, indexed by remediation
        ...
        crowdsec:
            ...
            # Bouncer API key, created with "cscli bouncers add <name>". Required
...
- `symfony/console`: the `vigie:*` commands. Without it, they aren't registered.
- `symfony/uid`: UUIDv7 request correlation ids and event ids. Without it, both fall back to random bytes,
  still unique but not sortable/timestamped.
