...
- `hostname` → `host.hostname`, omitted unless `iq2i_vigie.hostname` resolves to a value (auto-detected
  with `gethostname()` by default); see [doc/multi-server.md](multi-server.md#telling-nodes-apart)
...
- `route` / `firewall` / `sessionId` → `vigie.route` / `vigie.firewall` / `vigie.session_id` (always an
  HMAC)
- `subject` → `vigie.subject.type` / `vigie.subject.id` / `vigie.subject.owner`; `owner`, when present,
  also joins `related.user` and goes through the same redaction as `user.name`/`user.hash` (see
  [doc/recording.md](recording.md#subject)) — useful to flag an IDOR by comparing it against the
  acting user
...
which fields are always present, is the published [JSON Schema](schema/activity.json).
...
built-in ECS support, plus scenarios to actually act on them. [crowdsec/](../crowdsec) ships both: a parser
and six ready-to-install scenarios (login brute force, credential stuffing, access-denied probing, a CSRF
wave, impersonation abuse, IDOR probing), plus the acquisition snippet and install steps. See
[crowdsec/README.md](../crowdsec/README.md), in particular its warning about `record.ip_address: anonymize`
(the default) making every IP-based scenario there useless: see also
[The pseudonymization pitfall](threat.md#the-pseudonymization-pitfall).

For a fleet of load-balanced servers, shipping straight to syslog instead of one file per node is usually a
better fit than the file-based acquisition in [crowdsec/](../crowdsec). See
[doc/multi-server.md](multi-server.md#sending-to-syslog) for the two transports, and a caveat on the same
parser when CrowdSec's `syslog` datasource is the one acquiring the line instead of a file.
