...
// Imported only when threat.provider is "crowdsec" and symfony/http-client is installed, so
// referencing HttpClient/HttpClientInterface here is always safe. Most arguments below are
// placeholders, overridden by loadThreatExtension().
return static function (ContainerConfigurator $container): void {
    $services = $container->services();

    // A scoped client, its base URI/timeout overridden by loadExtension(). Skipped entirely in
    // favor of the service named by threat.crowdsec.http_client, when one is configured.
