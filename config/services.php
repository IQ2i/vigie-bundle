...
return static function (ContainerConfigurator $container): void {
    ...
    // IQ2iVigieBundle::loadExtension() once iq2i_vigie.* is processed.

    // Referenceable by id from a project's own monolog.yaml.
    $services->set('iq2i_vigie.formatter.ecs', EcsFormatter::class);

    // Same document, without a trailing newline, for a handler that already frames one message per call.
    $services->set('iq2i_vigie.formatter.ecs.syslog', EcsFormatter::class)
        ->arg('$appendNewline', false);
    ...
    // Bound explicitly: an unset argument would make the container try to autoload TokenStorageInterface.
    ...
    if (interface_exists(CsrfTokenManagerInterface::class)) {
        ...
        $services->set(RecordingCsrfTokenManager::class)
            // Negative priority: DecoratorServicePass wraps decorators from highest to lowest priority, so
            // this one ends up outermost, wrapping SameOriginCsrfTokenManager's own decoration too.
